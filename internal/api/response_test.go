...
func TestOKResponse(t *testing.T) {
	...
	t.Run("succesful http200 json response", func(t *testing.T) {
		...
		api.OKResponse(recorder, sample)
...
func TestErrorResponse(t *testing.T) {
	t.Run("json response for a given http status code", func(t *testing.T) {
		recorder := httptest.NewRecorder()
		api.ErrorResponse(recorder, http.StatusInternalServerError, "Some error occurred")
...
func TestOKResponse_InvalidPayload(t *testing.T) {
	recorder := httptest.NewRecorder()

	invalidJson := make(chan int)

	err := api.OKResponse(recorder, invalidJson)

	assert.NotNil(t, err)
	assert.ErrorIs(t, err, api.ErrJSONResponseSerialization)
}

func TestErrorResponse_EmptyMessage(t *testing.T) {
	recorder := httptest.NewRecorder()

	api.ErrorResponse(recorder, http.StatusBadRequest, "")
